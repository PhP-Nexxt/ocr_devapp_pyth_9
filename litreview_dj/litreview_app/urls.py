...
urlpatterns = [
    ...
    ),  # certification de la connexion
    path("logout/", auth_views.LogoutView.as_view(next_page="login"), name="logout"),
    path("dashboard/", views.dashboard, name="dashboard"),
    path("register/", views.user_register, name="register"),
    path(
        "create_ticket/", views.create_ticket, name="create_ticket"
    ),  # Le premier parametre peut etre modifié(Seo par Ex)
    path(
    ...
    path(
        "update_review/<int:review_id>", views.update_review, name="update_review"
    ),  # Parametre1=url/Parametre2=nom fonction/Parametre3=nom template html
