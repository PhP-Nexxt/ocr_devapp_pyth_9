...
def create_review(request, ticket_id):
    ticket = Ticket.objects.get(
        id=ticket_id
    )  # Recuperation du ticket id pour ajout de critique
    if request.method == "POST":
        review_form = ReviewForm(request.POST, request.FILES)
        if review_form.is_valid():
            new_review = review_form.save(commit=False)
            new_review.ticket = (
                ticket  # Assuming your Review model has a foreign key to Ticket model
            )
...
def update_review(request, review_id):
    review = Review.objects.get(
        id=review_id
    )  # Recuperation de la critique id pour ajout de critique
...
def delete_review(request, review_id):
    review = Review.objects.get(
        id=review_id
    )  # Recuperation de la critique id pour suppression
    review.delete()
